...
function PredictionForm() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        const response = await fetch('http://127.0.0.1:5000/predictParkinsons/', {
            method: 'POST',
            headers: {
        ...
        const advice = data.prediction ? 'Please consult a neurologist for further evaluation and treatment.' : 'Continue maintaining a healthy lifestyle and schedule regular check-ups.';
        setPrediction({
            message: `Prediction: ${data.prediction ? 'Likely have Parkinson\'s Disease' : 'Do not have Parkinson\'s Disease'}`,
            hasParkinsons: data.prediction,
            advice: advice
        });
    };
    ...
    const downloadPDF = async () => {
        ...
        pdf.save('Parkinson_Disease_Prediction_Report.pdf');
        ...
        <div className="flex relative dark:bg-main-dark-bg">
            <div className={"dark:bg-main-dark-bg bg-main-bg min-h-screen w-full"}>
                <form onSubmit={handleSubmit} className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg p-6 rounded-2xl w-full md:w-1200 mb-4 mx-auto">
                    <h1 className='text-white' style={{fontSize: '24px'}}>Parkinson Disease Prediction</h1>
                    <p className='text-gray-400'>This Parkinson Disease Prediction model will determine whether an individual has the Parkinson disease or not with an accuracy of 98.3%</p>
                    <br />
                ...
                {prediction && (
                    <div className={`mt-4 p-4 text-lg font-semibold rounded-md w-full md:w-1200 mx-auto ${prediction.hasParkinsons ? 'bg-red-200 text-red-700' : 'bg-green-200 text-green-700'}`}>
                        {prediction.message}
                        ...
                        <div className="flex justify-center mt-4">
                            ...
                            <CSVLink data={csvData} filename="Parkinson_Disease_Prediction_Report.csv" className="bg-blue-600 text-white font-bold p-3 rounded mr-2 hover:bg-blue-700 transition duration-300">
                                Download CSV
                            </CSVLink>
                            <a href={`data:text/json;charset=utf-8,${encodeURIComponent(JSON.stringify(jsonData, null, 2))}`}
                               download="Parkinson_Disease_Prediction_Report.json"
                ...
                <div id="report" style={{ padding: '20px', fontFamily: 'Arial, sans-serif', display: 'none' }}>
                    <div style={{ backgroundColor: '#ddeeff', padding: '10px', borderRadius: '5px', marginBottom: '20px' }}>
                        ...
                        <h2 style={{ color: '#003366' }}>Parkinson Disease Prediction Report</h2>
                    </div>
