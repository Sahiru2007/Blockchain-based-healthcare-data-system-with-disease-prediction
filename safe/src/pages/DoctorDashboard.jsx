...
const Ecommerce = () => {
  ...
  useEffect(() => {
    const fetchDetailedEarnings = async () => {
      try {
        const response = await fetch('http://localhost:8050/api/appointments/detailedEarnings');
        if (response.ok) {
          const data = await response.json();
          setAppointments(data.appointments);
          setTotalEarnings(data.totalEarnings);
        } else {
          console.error('Failed to fetch detailed earnings:', response.statusText);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchTelemedicineSessions = async () => {
      try {
        const response = await fetch('http://localhost:8050/api/appointments/telemedicine-sessions');
        if (response.ok) {
          const data = await response.json();
          setTelemedicineSessions(data);
        } else {
          console.error('Failed to fetch telemedicine sessions:', response.statusText);
        }
      } catch (error) {
  ...
  const generateReport = () => {
    ...
    doc.text(`Total Earnings: $${totalEarnings.toFixed(2)}`, 14, doc.autoTable.previous.finalY + 10);
        ...
        <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg m-3 p-4 rounded-2xl md:w-780  ">
          ...
          <div className="mt-10 flex gap-10 flex-wrap justify-center">
            <div className=" border-r-1 border-color m-4 pr-10">
              <div>
                <p>
                  <span className="text-3xl font-semibold">Rs. {totalEarnings}</span>
      ...
      <div className="flex gap-10 m-4 flex-wrap justify-center">
        <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg p-6 rounded-2xl">
          <div className="flex justify-between items-center gap-2">
            <p className="text-xl font-semibold">Recent Telemedicine Sessions</p>
      ...
      <div key={session._id} className="flex justify-between mt-4">
        ...
        <div className="bg-white dark:text-gray-200 dark:bg-secondary-dark-bg p-6 rounded-2xl w-96 md:w-760">
          <div className="flex justify-between items-center gap-2 mb-10">
            <p className="text-xl font-semibold">Prediction Overview</p>
            <DropDown currentMode={currentMode} />
          </div>
